...
static bool scan_raw_text(Scanner *scanner, TSLexer *lexer) {
    ...
    array_back(&scanner->tags)->type == STYLE  ? "</STYLE"  :
...
static bool scan_implicit_end_tag(Scanner *scanner, TSLexer *lexer) {
    ...
    if (is_closing_tag) {
        ...
        parent &&
        (
            !tag_can_contain(parent, &next_tag) || lexer->eof(lexer)
        )
    ) {
        pop_tag(scanner);
        lexer->result_symbol = IMPLICIT_END_TAG;
...
static bool scan_start_tag_name(Scanner *scanner, TSLexer *lexer) {
    ...
    switch (tag.type) {
        ...
        case RAW:
            array_push(&scanner->tags, tag);
            lexer->result_symbol = RAW_START_TAG_NAME;
            break;
        case SCRIPT:
            array_push(&scanner->tags, tag);
...
void *tree_sitter_gox_external_scanner_create() {
    Scanner *scanner = (Scanner *)ts_calloc(1, sizeof(Scanner));
    return scanner;
}

bool tree_sitter_gox_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    Scanner *scanner = (Scanner *)payload;
    return scan(scanner, lexer, valid_symbols);
}

unsigned tree_sitter_gox_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize(scanner, buffer);
}

void tree_sitter_gox_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
    deserialize(scanner, buffer, length);
}

void tree_sitter_gox_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    for (unsigned i = 0; i < scanner->tags.size; i++) {
        tag_free(&scanner->tags.contents[i]);
    }
